...
void test1Both(int port, int serverSocket_) {
  ...
  struct sockaddr_storage s1_addr{};
  size_t addrLen = sizeof(sockaddr_storage);
  int s1 = accept(serverSocket_, (sockaddr*) &s1_addr, &addrLen);
  if (s1 < 0) {
    close(c1);
    close(c2);
  ...
  struct sockaddr_storage s2_addr{};
  addrLen = sizeof(sockaddr_storage);
  int s2 = accept(serverSocket_, (sockaddr*)&s2_addr, &addrLen);
  if (s2 < 0) {
    close(c1);
    close(c2);
    close(s1);
  ...
  if (s1_addr.ss_family == AF_INET6) {
    if (s2_addr.ss_family != AF_INET) {
        throw std::runtime_error("Both connections are IPv6?????");
    }
